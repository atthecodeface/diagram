...
(*m Styleable_value_ref *)
  ...
  let get_value_as_floats t =  Value.svalue_as_floats t.value

  (*f get_value_as_float t - get the value as a float *)
  let get_value_as_float t  =  Value.svalue_as_float  t.value

  (*f get_value_as_ints t - get the value as an array of ints *)
  let get_value_as_ints t   =  Value.svalue_as_ints   t.value
  ...
  let set_default_from_string t stype value =
    t.default_value <- Value.svalue_from_string stype value

  (*f reset *)
  let reset t =     
    t.longest_rule <- 0;
    if (t.default_value == Value.sv_none) then
      (
